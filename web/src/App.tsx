...
import { Game, GameBanner } from './components/GameBanner';
...
export const App = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:4000/games')
            ...
            .then(setGames)
            .catch(() => alert('Não foi possível buscar os jogos!'));
    }, []);
        ...
        <div className="max-w-[1344px] mx-auto flex flex-col items-center my-20">
            ...
            <div className="grid grid-cols-6 gap-6 mt-16">
                {games?.map((item, index) => (
                    <GameBanner key={index} bannerUrl={item.bannerUrl} title={item.title} adsCount={item._count.ads} />
                ))}
            </div>
                    ...
                    <Dialog.Content className="fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25">
                        ...
                        <form className="mt-8 flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <Input id="name" type="text" placeholder="Seu nome dentro do game?" />
                            </div>
